...
def game_scene():
    ...
    score_text.clear()
    score_text.cursor(5 * constants.SPRITE_SIZE,10)
    score_text.cursor("Score: {0}".format(score))
    text.append(score_text)

    combo = 0
    combo_text = stage.Text(width=29, height=14)
    combo_text.clear()
    combo_text.cursor(5 * constants.SPRITE_SIZE,20)
    ...
    def show_note():
        ...
        for note_number in range(len(notes)):
            if notes[note_number].x < 0:
                lane_picked = random.randint(1, 4)
    ...
    # repeat forever, game loop
    # repeatedly checks for player input, collisions, moves notes
    while True:
        # downward note movement
        for note_number in range(len(notes)):
            if notes[note_number].x > 0:
